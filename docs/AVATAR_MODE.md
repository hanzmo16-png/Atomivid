...
Estado: **arquitectura, DOS proveedores (HeyGen y D-ID), UI y wiring del pipeline implementados y probados (fixture/mocks); ninguna llamada real fue posible con ninguno de los dos (sin `HEYGEN_API_KEY` ni `DID_API_KEY`).** El modo "visual" (existente) sigue siendo el único disponible para usuarios reales — `AVATAR_MODE_ENABLED=false` por defecto, y `AVATAR_PROVIDER` sigue en `fixture` salvo configuración explícita. **NI el adaptador HeyGen NI el adaptador D-ID deben considerarse production-ready** — ver "Lo que no se pudo confirmar" (HeyGen) y el comentario de cabecera de `providers/avatar/did.ts` (D-ID) — son bloqueos reales, no cosméticos. **Los tests con fetch mockeado NUNCA prueban compatibilidad real con la API del proveedor** — solo prueban que el código hace lo que el mock espera; la única prueba real es una llamada real, acotada y autorizada explícitamente (ver "Próxima prueba pagada" más abajo).

## D-ID (`providers/avatar/did.ts`) — implementado, CORREGIDO contra documentación oficial (parcial)

Acceso directo a `docs.d-id.com` y `www.d-id.com` sigue bloqueado en este entorno (confirmado de nuevo el 2026-09-17: EGRESS_BLOCKED en todo el dominio, no solo `/docs`) — pero WebSearch devolvió resultados que citan/resumen directamente varias páginas oficiales, lo bastante específicos para corregir DOS bugs reales que la versión anterior tenía y que habrían hecho fallar cualquier llamada real desde el primer intento:
...
Diferencia arquitectónica clave frente a HeyGen: D-ID no tiene una fase de "entrenamiento" de avatar separada — `createAvatar()` solo sube la foto y devuelve `status: "completed"` de inmediato; `checkAvatarStatus()` es un no-op que siempre confirma "completed". **No actives `DID_API_KEY` en producción** sin antes ejecutar la prueba real controlada descrita en "Próxima prueba pagada".
