...
export default async function VideoResultPage({
  ...
  const { data } = await supabase
    .from("video_requests")
    .select(
      "id, user_id, topic, style, duration_seconds, language, status, video_path, error_message, script_json, progress_stage, render_attempts, render_started_at, created_at",
    )
    .eq("id", id)
    .eq("user_id", user.id)
