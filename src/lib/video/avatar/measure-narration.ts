...
export async function measureNarrationSeconds(bytes: Buffer): Promise<number> {
  ...
  try {
    ...
    const { stdout } = await run("ffprobe", ["-v", "error", "-show_entries", "format=duration:stream=codec_type", "-of", "json", file], { timeout: 15000, maxBuffer: 4096 });
    ...
    await run("ffmpeg", ["-v", "error", "-xerror", "-protocol_whitelist", "file,pipe", "-i", file, "-map", "0:a:0", "-f", "null", "-"], { timeout: 20000, maxBuffer: 4096 });
    return seconds;
  } finally {
