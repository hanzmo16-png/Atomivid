...
import { REEL_ANIMATION, animationClipCostUsd, buildContinuityBible, planAnimatedShots, planSceneAnimation, scenesTooLongForClip } from "./animation";
import { checkAnimationAvailability, evaluateDirectionReadiness } from "./readiness";
import { AnimatedClipUncertainError, animationLedgerKey, animationMarkerPath, looksLikeMp4, prepareAnimationInputImage, resolveAnimatedClipForScene } from "./animated-clip";
...
const SCRIPT: GeneratedScript = {
  title: "El faro",
  segments: [
    { text: "Nadie sabe qué pasó aquella noche en el faro.", visualQuery: "old lighthouse at night", visualConcepts: ["old lighthouse at night", "stormy coast"], energy: "low" },
    { text: "El guardián desapareció sin dejar rastro.", visualQuery: "empty stairway", energy: "medium" },
    { text: "Entonces encontraron la puerta cerrada por dentro.", visualQuery: "locked wooden door", energy: "high" },
  ],
};
...
test("plan: cada escena declara narración, sujeto, acción, estados, referencia, constantes y encuadre; la clave cambia con la imagen", () => {
  ...
  const spec = planSceneAnimation({ sceneIndex: 0, segment: SCRIPT.segments[0], energy: "low", intent: "suspense", bible, referenceImagePath: "req/scene-0-animbase-a.png", referenceImageKey: "a" });
  ...
  const other = planSceneAnimation({ sceneIndex: 0, segment: SCRIPT.segments[0], energy: "low", intent: "suspense", bible, referenceImagePath: "req/x.png", referenceImageKey: "b" });
...
async function resolveClip(s: ReturnType<typeof memoryStorage>, provider: VideoProvider, ledger?: PaidLedger) {
  ...
  return resolveAnimatedClipForScene({ supabase: s.client, bucket: "videos", requestId: "req", spec, inputImage: input, videoProvider: provider, maxCostUsd: animationClipCostUsd(), signedUrlTtlSeconds: 60, ledger });
}
...
test("clip: la ilustración de la escena viaja como imagen de entrada (recortada a 9:16), con reserva guardada ANTES de llamar", async () => {
...
test("clip con el adaptador real de Veo (fetch simulado): la imagen de entrada llega como bytes base64 a predictLongRunning", async () => {
  ...
  assert.notDeepEqual(inputBytes, png, "se envía la versión recortada a 9:16");
