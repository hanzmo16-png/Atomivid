 ...
 */
// Tamaño histórico (Shorts/Avatar) — sin cambios para request.aspectRatio === "9:16".
const PORTRAIT_SIZE = { width: 1080, height: 1920 };
// Agregado para Long Form (aspectRatio: "16:9") — nunca se usa para un pedido "9:16".
const LANDSCAPE_SIZE = { width: 1920, height: 1080 };

function sizeFor(aspectRatio: ImageGenerationRequest["aspectRatio"]) {
  return aspectRatio === "16:9" ? LANDSCAPE_SIZE : PORTRAIT_SIZE;
}

function svgDataUri(prompt: string, seed: number, size: { width: number; height: number }): string {
...
export const fixtureImageProvider: ImageProvider = {
  ...
  async generateImage(request: ImageGenerationRequest): Promise<GenerativeAsset> {
    ...
    const uri = svgDataUri(request.prompt, counter, size);
