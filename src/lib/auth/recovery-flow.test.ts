...
/**
 * SDK real de Supabase (@supabase/ssr) + cookies en memoria, con TODO el
 * HTTP interceptado — cero red real, cero cuenta/correo/contraseña reales.
 * Confirma contra el SDK instalado (no solo contra tipos estáticos) que
 * `exchangeCodeForSession()` realmente devuelve `redirectType: "recovery"`
 * para un código de recuperación — src/app/auth/callback/route.ts depende
 * de este campo en runtime aunque `AuthTokenResponse` no lo declare
 * estáticamente (ver el acceso vía cast en ese archivo).
 */
function fixture() {
    ...
    global: {
      fetch: async (input, init) => {
        ...
        if (url.pathname.endsWith("/token")) {
          if (body.auth_code !== "valid-code") {
            return new Response(JSON.stringify({ error_code: "otp_expired", msg: "Expired" }), { status: 400 });
          }
...
test("ciclo de vida SDK de recuperación: solicitud, intercambio PKCE, prueba, actualización y cierre de sesión", async () => {
  const { sdk, calls, jar } = fixture();
  assert.equal(
    (
      await sdk.auth.resetPasswordForEmail("owner@example.test", {
        redirectTo: "https://atomivid.example/auth/callback?next=/reset-password",
      })
    ).error,
    null,
  );
...
test("un código expirado no produce sesión ni autorización de recuperación", async () => {
  const { sdk } = fixture();
  await sdk.auth.resetPasswordForEmail("owner@example.test");
