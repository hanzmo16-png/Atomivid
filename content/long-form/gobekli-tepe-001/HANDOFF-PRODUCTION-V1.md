...
```
4348aed  feat(long-form): add read-only review gallery for existing Visual Test V2 images
```
...
| `85d7141` | Actualización del handoff (referencias de commit) tras el checkpoint anterior — sin cambios de código. |
| `03e1344` | Acceso directo en el nav del dashboard ("Dry Run Long Form") hacia la página de DRY_RUN — mismo gate Long Form/allowlist, solo navegación (`<Link>`, sin fetch propio). Sin cambios al pipeline 9:16 ni a facturación/suscripciones. Ver sección 23. Validado con 687/687 tests. Sin deployment manual — queda para la integración automática de Vercel al hacer push. |
| `df71e77` | Generación REAL controlada, AUTORIZADA explícitamente por el usuario, de exactamente las 3 imágenes aprobadas del Visual Test V2 — nuevo endpoint `POST /api/long-form/visual-test-v2/real` y segundo botón en la misma página, completamente separados del DRY_RUN (que queda intacto). Almacenamiento durable en el bucket `videos` de Supabase Storage (ya existente, reutilizado). Ver sección 24. Validado con 729/729 tests. Código listo y commiteado — NINGUNA llamada real se ejecutó desde este agente; el usuario dispara la generación manualmente desde la UI en producción. Sin deployment manual — queda para la integración automática de Vercel al hacer push. |
| `4348aed` | **Este checkpoint.** Las 3 imágenes reales YA fueron generadas por el usuario (costo real confirmado: US$0.1681) — se añade una galería de SOLO LECTURA (`VISUAL TEST V2 — REVISIÓN`) que muestra las 3 imágenes ya existentes en Storage (URLs firmadas de corta duración, bucket privado sin cambios), para poder evaluarlas visualmente. Cero generación, cero llamada a OpenAI, ledger sin tocar. Ver sección 25. Validado con 745/745 tests. Sin deployment manual — queda para la integración automática de Vercel al hacer push. |
...
## 22. UI administrativa mínima y temporal para el DRY_RUN (checkpoint posterior a `fdf6680`)

**Por qué:** el checkpoint anterior dejó `POST /api/long-form/visual-test-v2` listo, pero ejecutarlo desde un móvil requería la consola de JavaScript del navegador — impráctico. Este checkpoint añade una página interna con un único botón que hace exactamente esa misma llamada, nada más.

**Página nueva:** `/dashboard/long-form/visual-test-v2` (`src/app/dashboard/long-form/visual-test-v2/page.tsx`). Mismo gate que el resto de Long Form — `isLongFormEnabled()` + `isLongFormAllowlisted(user)` (`access.ts`, sin código nuevo de autorización) — usuario no autorizado o anónimo → `404` (mismo patrón que `/dashboard/avatar/prepare`), nunca revela que la página existe.

**Componente `DryRunButton.tsx`** (`"use client"`): un único botón, texto exacto "Ejecutar Dry Run Long Form". Al pulsarlo hace `POST /api/long-form/visual-test-v2` con el body LITERAL fijo `{ mode: "dry_run" }` — no hay ningún `<input>`/`<textarea>`/`<select>` en el componente, así que estructuralmente no hay forma de que el usuario escriba un prompt, una cantidad o un costo que llegue a la request. No existe ningún botón ni código para REAL generation en esta página. La respuesta se formatea con una función pura separada (`src/lib/video/long-form/visual-test-v2-display.ts`, `summarizeDryRunResponse`) que copia EXPLÍCITAMENTE solo los campos esperados y conocidos-seguros — nunca hace passthrough genérico del body — y en caso de error muestra `status` + el mensaje ya seguro que devuelve el propio endpoint (nunca un secreto: el endpoint nunca los incluye, ver sección 21).

**Tests (15 nuevos, todos pasando):**
- `visual-test-v2-display.test.ts` (7): reporte 200 completo, errores 401/403/400/500 con mensaje seguro, body `null`/no-objeto nunca lanza, un campo inesperado con apariencia de secreto en el body NUNCA se propaga al resultado (solo se copian campos conocidos), `costGuardBlockReason` se incluye cuando aplica, shots mal formados no rompen (valores por defecto seguros).
- `visual-test-v2-ui-structural.test.ts` (8, lee el código fuente de `page.tsx`/`DryRunButton.tsx`, mismo principio que `render-route-cas.test.ts`): el gate usa `isLongFormEnabled`/`isLongFormAllowlisted`/`notFound`; el POST va exclusivamente a `/api/long-form/visual-test-v2` con el body literal `{ mode: "dry_run" }`; el botón tiene el texto exacto pedido; no hay ningún campo de formulario; no aparece `mode:"real"` en ningún lado; ni el botón ni la página leen/muestran `process.env`; el botón no importa ningún proveedor real de imagen/voz/footage; la página no expone ningún control de generación real.

**Validación de este checkpoint:** `npm run typecheck` limpio, `npm run lint` limpio (0 warnings), `npm run test:unit` 682/682 (667 previos + 15 nuevos), `npm run build` exitoso — la nueva ruta `/dashboard/long-form/visual-test-v2` aparece listada como `ƒ` (server-rendered). `git diff --stat` confirma cero cambios en `remotion/VerticalReel.tsx`, `src/lib/video/generate-video.ts` y `src/app/api/generate` — pipeline 9:16 intacto. `VISUAL_TEST_V2_REAL_MODE_LOCKED = true` reconfirmado sin cambios (constante de código, no env var) antes y después de este checkpoint. Cero llamadas pagadas: no se desplegó manualmente (queda para la integración automática de Vercel al hacer push), no se generaron imágenes, no se llamó a ninguna API real.

## 23. Acceso directo en el nav del dashboard (checkpoint posterior a `85d7141`)

**Por qué:** la página del DRY_RUN (sección 22) ya existía pero solo era alcanzable escribiendo la URL a mano — se pidió un acceso directo visible en el dashboard, gateado por el mismo control Long Form/allowlist, que SOLO navegue (sin hacer ningún fetch/POST por su cuenta).

**Cambio:** `src/app/dashboard/layout.tsx` calcula `showLongFormDryRun = isLongFormEnabled() && isLongFormAllowlisted(user)` (mismas funciones de `access.ts`, sin código de autorización nuevo) y, solo si es `true`, muestra un `NavLink` con el texto exacto "Dry Run Long Form" hacia `/dashboard/long-form/visual-test-v2` — en el nav de escritorio (junto a Historial/Facturación) y en la fila de nav de móvil. El enlace es un `<Link>` de Next.js puro: no dispara ningún `fetch`, no toca `/api/long-form/visual-test-v2` directamente — la propia página de destino es la que ya tenía el botón que sí hace el POST (sección 22).

**Tests (5 nuevos, todos pasando):** `visual-test-v2-nav-structural.test.ts` (lee el código fuente de `layout.tsx`, mismo principio que las pruebas estructurales anteriores): el gate usa `isLongFormEnabled`/`isLongFormAllowlisted`; aparece el texto exacto "Dry Run Long Form"; el enlace apunta exclusivamente a `/dashboard/long-form/visual-test-v2` y `layout.tsx` no contiene ningún `fetch(...)` ni referencia directa a `/api/long-form/visual-test-v2`; las DOS apariciones del enlace (escritorio y móvil) están condicionadas por `showLongFormDryRun`; sin imports de proveedores reales ni `process.env`.

**Validación de este checkpoint:** `npm run typecheck` limpio, `npm run lint` limpio (0 warnings), `npm run test:unit` 687/687 (682 previos + 5 nuevos), `npm run build` exitoso. `git diff --stat` confirma cero cambios en `remotion/VerticalReel.tsx`, `src/lib/video/generate-video.ts`, `src/app/api/generate`, `src/app/dashboard/billing` y `src/lib/billing` — pipeline 9:16 y facturación/suscripciones intactos. `VISUAL_TEST_V2_REAL_MODE_LOCKED = true` reconfirmado sin cambios. Cero llamadas pagadas: solo se agregó un enlace de navegación, ningún código nuevo llama a ninguna API. Sin deployment manual — queda para la integración automática de Vercel al hacer push.

## 24. Generación REAL controlada de las 3 imágenes del Visual Test V2 (checkpoint posterior a `03e1344` — AUTORIZADO por el usuario, código listo pero NO ejecutado)

**Autorización:** el usuario confirmó el resultado real del DRY_RUN en Vercel (OPENAI_API_KEY disponible, 3/3 shots correctos, costo estimado $0.15, dentro del cost guard, REAL mode bloqueado, `paidApisCalled: false`) y autorizó explícitamente la generación REAL de EXACTAMENTE esas 3 imágenes (`b1-s4`, `b4-s2`, `b8-s5`), nada más. Este checkpoint deja el mecanismo completamente construido, testeado y commiteado — **la llamada real la dispara el usuario manualmente desde la UI en producción, después de este deployment**, no este agente.

**El DRY_RUN queda 100% intacto:** `git diff --stat` confirma cero cambios en `visual-test-v2.ts`, `visual-test-v2-runtime.ts` y `video-cost-guard.ts` — `VISUAL_TEST_V2_REAL_MODE_LOCKED = true` en `visual-test-v2-runtime.ts` sigue sin tocar; ese endpoint (`POST /api/long-form/visual-test-v2`) sigue sin ningún código de generación real.

**Almacenamiento durable elegido:** Supabase Storage, bucket **`videos`** — el MISMO bucket que ya usa Shorts/Avatar (`generate-video.ts`, `STORAGE_BUCKET`) para subir imágenes reales generadas con este mismo proveedor OpenAI vía `visual-resource-resolver.ts`. No se crea ningún bucket ni tabla nueva — sin migración SQL, sin coordinación de infraestructura aparte. Rutas usadas (prefijo `long-form/gobekli-tepe-001/`):
- `visual-test-v2/<shotId>-<idempotencyKey>.png` — la imagen.
- `state/cost-ledger.json` — el mismo `CostLedger` de `video-cost-guard.ts` (tipo reutilizado sin cambios), ahora persistido en Storage en vez de disco local (que en Vercel es efímero y no serviría para el hard stop).
- `state/visual-test-v2/<idempotencyKey>.json` — registro por shot (STARTED/COMPLETED), mismo principio que `tts-cache.ts` pero con Supabase Storage como backend.

**Archivos nuevos (todos additive — cero cambios en archivos existentes de Visual Test V2 o del cost guard):**
- `src/lib/video/long-form/visual-test-v2-storage.ts` — I/O puro contra Supabase Storage (ledger, registro por shot, subida/descarga de imagen, checksum SHA-256).
- `src/lib/video/long-form/visual-test-v2-real-confirm.ts` — solo exporta `VISUAL_TEST_V2_REAL_CONFIRM_VALUE = "GENERATE_3_VISUAL_TEST_V2_IMAGES"`, aislado sin ningún otro import para que el botón cliente pueda importarlo sin arrastrar código server-only al bundle del navegador.
- `src/lib/video/long-form/visual-test-v2-real.ts` — orquestación: `runVisualTestV2RealGeneration(supabase, bucket, imageProvider = openaiImageProvider)`. `imageProvider` es inyectable (mismo patrón que `synthesizeBeat` en `timeline.ts`) — en producción SIEMPRE es `openaiImageProvider` real (nunca cae a fixture en silencio), los tests inyectan un mock contador.
- `src/app/api/long-form/visual-test-v2/real/route.ts` — `POST` nuevo y separado del DRY_RUN, mismo gate `assertLongFormAccess`.
- `src/lib/video/long-form/visual-test-v2-real-display.ts` — formateo puro de la respuesta para la UI (allow-list explícita de campos, igual que el del DRY_RUN).
- `src/app/dashboard/long-form/visual-test-v2/RealGenerateButton.tsx` — segundo control en la misma página, claramente separado.
- `page.tsx` (modificado): calcula `shotIds`/costo estimado/hard stop en el SERVIDOR desde el manifest real y los pasa como props de solo lectura al botón — nunca editables, nunca hardcodeados en el cliente.

**3 shotIds bloqueados (cierre de código, no de configuración):** `VISUAL_TEST_V2_REAL_SHOT_IDS = ["b1-s4", "b4-s2", "b8-s5"] as const` en `visual-test-v2-real.ts` — ningún otro shotId es alcanzable desde este mecanismo; no hay parámetro de cantidad en ninguna parte del flujo.

**Prompts bloqueados:** se reutiliza `buildVisualTestV2Manifest()` de `visual-test-v2.ts` SIN NINGÚN CAMBIO — los prompts/negativos/estilo compartido (Visual Bible) son exactamente los ya aprobados. El body del `POST` es EXCLUSIVAMENTE `{ "confirm": "GENERATE_3_VISUAL_TEST_V2_IMAGES" }` (mismo criterio que `LONG_FORM_REAL_RUN_CONFIRM` en `mode.ts`: deliberadamente largo y específico) — cualquier otro campo (prompt, count, shotIds, mode) se rechaza con 400 antes de tocar nada.

**Costo estimado / hard stop:** reutilizados sin cambios de `video-cost-guard.ts` — $0.15 estimado (3 × $0.05), tope Visual Test V2 $0.50, hard stop VIDEO #001 $3.00. El chequeo es POR LOTE: se suma el costo estimado de TODOS los shots que haría falta generar y se valida contra el ledger persistido (Supabase) ANTES de llamar a OpenAI para cualquiera de ellos — nunca se generan 2 de 3 y se descubre a mitad de camino que el lote entero excedía el tope.

**Idempotencia:** identidad = el mismo `idempotencyKey` que ya calcula `buildVisualTestV2Manifest()` (hash de shotId+modelo+tamaño+calidad+prompt+negativo). Un shot con registro `COMPLETED` cuyo archivo en Storage pasa la validación de checksum (existe, no vacío, checksum coincide) → se reutiliza, CERO llamada a OpenAI. Un archivo corrupto o un registro sin archivo (faltante) → se trata como pendiente y se regenera. Verificado con tests dedicados (corrupción, archivo faltante, reuso parcial, reuso total).

**Comportamiento ante consumo incierto:** un registro en estado `STARTED` sin `COMPLETED` válido (crash a mitad de una llamada anterior) hace que `VisualTestV2UncertainCostStateError` aborte TODO el lote — incluidos los shots que en principio estarían seguros de generar — ANTES de llamar a OpenAI para ninguno. Requiere revisión manual (confirmar en el dashboard de OpenAI que no se cobró, y solo entonces borrar manualmente ese registro en Storage) antes de poder reintentar — nunca un reintento automático.

**UI:** en `/dashboard/long-form/visual-test-v2`, debajo del DRY_RUN existente (que se conserva sin cambios), un segundo bloque claramente separado (borde propio) muestra de forma fija — sin ningún campo editable — "3 imágenes: b1-s4, b4-s2, b8-s5", "Costo estimado: US$0.15", "Hard stop: US$0.50", y el botón **"Generar 3 imágenes — máximo US$0.50"**. Al pulsarlo, un `window.confirm()` nativo pide una segunda confirmación explícita antes de disparar el `POST`. Solo visible para el mismo gate Long Form/allowlist (usuario no autorizado → la página entera sigue devolviendo 404).

**Tests (nuevos, todos pasando):**
- `visual-test-v2-storage.test.ts` (11): round-trips de ledger/registro por shot, validación de archivo existente/faltante/corrupto, subida, checksum determinístico.
- `visual-test-v2-real.test.ts` (12): shots nuevos → genera los 3; segunda ejecución → reuso total, proveedor con 0 llamadas; `STARTED` incierto → aborta TODO el lote antes de llamar al proveedor; costo excede $0.50 → aborta antes de llamar; costo excede el hard stop $3.00 → aborta antes de llamar; proveedor no disponible → error explícito, 0 llamadas; reuso parcial (1 de 3) → exactamente 2 llamadas; archivo corrupto → regenera ese shot; registro sin archivo → se trata como pendiente; cost guard cuenta solo lo nuevo en un reintento parcial.
- `visual-test-v2-real-display.test.ts` (6): mapeo de respuesta 200/error, campo inesperado con apariencia de secreto NUNCA se propaga, entradas mal formadas no rompen.
- `visual-test-v2-real-ui-structural.test.ts` (13): gate de autorización en la ruta; body estricto (`confirm` exacto, nunca prompt/count/shotIds); delega en `runVisualTestV2RealGeneration`; nunca expone `error.cause` crudo; texto exacto del botón; POST exclusivo al endpoint correcto con el confirm importado (no un literal duplicado a mano); sin campos de formulario; props de solo lectura; sin `process.env` ni imports server-only en el componente cliente; página sigue con el mismo gate y con el DRY_RUN intacto; nada referencia el storyboard de 45 shots ni el orquestador de producción completa.
- Se corrigió además un test ya existente (`visual-test-v2-ui-structural.test.ts`) que afirmaba que la página "no expone ningún control de REAL generation" — ya no es cierto por diseño (este checkpoint lo autoriza explícitamente, con el cierre de 3 shots) — se actualizó para reflejar la nueva invariante correcta.

**Validación de este checkpoint:** `npm run typecheck` limpio, `npm run lint` limpio (0 warnings), `npm run test:unit` 729/729 (687 previos + 42 nuevos), `npm run build` exitoso — las nuevas rutas `/api/long-form/visual-test-v2/real` y (actualizada) `/dashboard/long-form/visual-test-v2` aparecen listadas; mismo único warning preexistente de `avatar/pipeline.ts` (se corrigió una importación que hubiera arrastrado a `tts-cache.ts`, con su propio código de disco, al bundle de esta ruta — ahora `computeChecksumSha256` está definido localmente en `visual-test-v2-storage.ts`, sin esa dependencia). `git diff --stat` confirma cero cambios en `remotion/VerticalReel.tsx`, `generate-video.ts`, `api/generate`, `dashboard/billing`, `lib/billing`, y en los 3 archivos del DRY_RUN (`visual-test-v2.ts`, `visual-test-v2-runtime.ts`, `video-cost-guard.ts`) — pipeline 9:16, facturación/suscripciones y DRY_RUN intactos.

**Cero consumo en este checkpoint:** el código queda listo, commiteado y push-eado, pero NINGUNA llamada real a OpenAI se ejecutó desde este agente. `runVisualTestV2RealGeneration` nunca se invocó fuera de los tests (que usan un proveedor mock contador, cero red). No hay deployment manual — queda para la integración automática de Vercel al hacer push, y el usuario ejecutará el botón manualmente en producción después de confirmar que el deployment está Ready.

## 25. Galería de revisión de las 3 imágenes ya generadas (checkpoint posterior a `df71e77` — LAS 3 IMÁGENES YA EXISTEN, generadas por el usuario en un checkpoint anterior, costo real acumulado confirmado por el usuario: US$0.1681)

**Problema resuelto:** la página ya ejecutaba la generación REAL correctamente, pero no mostraba las imágenes — solo el reporte JSON de texto. Este checkpoint añade una galería de SOLO LECTURA que muestra las 3 imágenes ya existentes en Storage, sin tocar la generación en absoluto.

**Cómo se recuperaron las imágenes existentes:** se reutilizó, SIN CAMBIOS, todo lo que ya existía — `buildVisualTestV2Manifest()` (para los `idempotencyKey`) y `readVisualTestV2ShotRecord()` (para los registros `COMPLETED` ya persistidos por `visual-test-v2-real.ts` en el checkpoint anterior). El módulo nuevo, `src/lib/video/long-form/visual-test-v2-gallery.ts` (`buildVisualTestV2Gallery`), SOLO lee — no escribe nada, no importa `openaiImageProvider` ni ningún otro proveedor real (verificado con test estructural sobre las líneas `import`). Para cada uno de los 3 `VISUAL_TEST_V2_REAL_SHOT_IDS` (mismo cierre de código que ya existía), si el registro es `COMPLETED` y tiene `storagePath`, firma una URL de corta duración (`createSignedUrl`, TTL 300s = 5 minutos) — un registro `STARTED`, sin `storagePath`, o cuya firma falla, simplemente se omite de la galería (nunca rompe la página, nunca dispara nada).

**Storage paths reutilizados (sin cambios):** `long-form/gobekli-tepe-001/visual-test-v2/<shotId>-<idempotencyKey>.png` (imagen) y `long-form/gobekli-tepe-001/state/visual-test-v2/<idempotencyKey>.json` (registro con costo real, dimensiones, provider/model/quality) — el bucket sigue siendo `videos` (Supabase Storage), sin ningún cambio de policy.

**Seguridad de acceso:**
- El bucket `videos` **ya era privado** desde la migración `0006_render_worker.sql` (`update storage.buckets set public = false where id = 'videos'`) — se confirmó, no se tocó ninguna policy ni se hizo público nada.
- La galería vive DENTRO de `page.tsx`, después del mismo gate `isLongFormEnabled() && isLongFormAllowlisted(user)` que ya protegía toda la página (usuario no autorizado → 404, ni siquiera se construye la galería) — verificado con un test estructural que confirma que la llamada a `buildVisualTestV2Gallery(` aparece en el código FÍSICAMENTE DESPUÉS de `notFound();`.
- Las URLs firmadas se generan server-side con `createServiceClient()` (mismo patrón ya usado en `/dashboard/avatar/prepare/page.tsx` y en `visual-resource-resolver.ts`) — el service role key nunca sale del servidor; lo único que llega al navegador es una URL firmada, de corta duración, específica de cada archivo.
- Ningún secreto (`OPENAI_API_KEY`, `SUPABASE_SERVICE_ROLE_KEY`) aparece nunca en el objeto que devuelve `buildVisualTestV2Gallery` — se construye explícitamente campo por campo (nunca un passthrough del registro completo), verificado con un test que serializa el resultado y confirma que esos strings nunca aparecen.

**Galería (`VISUAL TEST V2 — REVISIÓN`, en `page.tsx`, debajo de los dos botones existentes):** por cada imagen — imagen a ancho completo (útil en móvil para evaluar realismo/composición/iluminación/coherencia histórica/artefactos/manos/anacronismos/consistencia), `shotId`, `status: COMPLETED`, costo real, dimensiones (`widthPx`×`heightPx`), proveedor/modelo/calidad, y un enlace "Ver imagen completa" (la misma URL firmada, abierta en pestaña nueva). Sin resultados todavía → mensaje simple, sin romper nada.

**Seguridad — la página de revisión NO ejecuta nada:** la galería es una función `await` dentro del Server Component (se recalcula en cada carga/refresco, pero solo LEE registros ya existentes y firma URLs — nunca genera). El botón rojo de generación real (`RealGenerateButton.tsx`, sin cambios de este checkpoint) sigue exactamente igual: solo se dispara con `onClick={runReal}` tras un `window.confirm()` explícito — verificado con test estructural que confirma que no hay `useEffect` ni ninguna invocación de `runReal();` fuera del manejador del botón, así que cargar o refrescar la página nunca lo dispara.

**Tests (16 nuevos, todos pasando):**
- `visual-test-v2-gallery.test.ts` (9): TTL corto; galería vacía sin registros; los 3 shots `COMPLETED` aparecen en el orden aprobado con `signedUrl`; solo se consideran los 3 shotIds aprobados (el módulo ni siquiera itera sobre otros); un `STARTED` se omite; un `COMPLETED` sin `storagePath` se omite; una firma fallida se omite sin romper la galería; el TTL pasado se usa realmente en `createSignedUrl`; cada item solo trae los campos esperados (ningún secreto).
- `visual-test-v2-gallery-structural.test.ts` (7): sin imports de proveedores reales; nunca escribe nada; la galería se calcula después del gate de autorización; encabezado exacto de la sección; se muestran todos los metadatos pedidos; `RealGenerateButton` no se dispara solo (sin `useEffect`, `runReal` pasado por referencia); ambos botones existentes se conservan.

**Validación de este checkpoint:** `npm run typecheck` limpio, `npm run lint` limpio (0 warnings), `npm run test:unit` 745/745 (729 previos + 16 nuevos), `npm run build` exitoso — mismo único warning preexistente de `avatar/pipeline.ts`, sin ningún warning nuevo. `git diff --stat` confirma cero cambios en `remotion/VerticalReel.tsx`, `generate-video.ts`, `api/generate`, `dashboard/billing`, `lib/billing`, los 3 archivos del DRY_RUN, y los archivos de la generación REAL (`visual-test-v2-real.ts`, `visual-test-v2-storage.ts`, `real/route.ts`, `RealGenerateButton.tsx`) — todo lo anterior queda intacto; solo se modificó `page.tsx` (para añadir la sección de galería) y se agregaron `visual-test-v2-gallery.ts` + 2 archivos de test.

**Confirmación OpenAI calls = 0 / additional cost = US$0.00:** ningún archivo de este checkpoint importa `openaiImageProvider` (verificado con test estructural sobre las líneas `import`). `buildVisualTestV2Gallery` nunca escribe en el ledger de costo ni en ningún registro — solo lee. El ledger sigue reflejando exactamente el gasto real ya confirmado por el usuario (US$0.1681 acumulados) sin ningún cambio de este checkpoint.

## 26. PRODUCCIÓN REAL COMPLETA de VIDEO #001 vía GitHub Actions (checkpoint posterior a `04f684e`/`d5c7686`/`3c7692f` — MP4 FINAL ENTREGADO, run 35998403448)

**Objetivo del checkpoint:** terminar VIDEO #001 hasta MP4 final + QC, ejecutando la producción real (8 imágenes AI_RECREATION restantes + 13 beats TTS + stock + composición de 45 shots + render 16:9 + QC) EXCLUSIVAMENTE vía GitHub Actions (nunca local), sobre autorización explícita del usuario. Resultado: **MP4 final generado, QC objetivo PASA, artifact descargable.**

**Gaps de arquitectura corregidos ANTES de poder ejecutar nada real** (los dos que bloqueaban desde el checkpoint anterior):
- **Ledger único durable:** `scripts/produce-long-form-video.ts`, en su rama `isVideo001RealRun` (activa solo cuando `storyboard.videoId === "gobekli-tepe-001" && args.mode === "real"`), usa el MISMO ledger de Supabase Storage (`long-form/gobekli-tepe-001/state/cost-ledger.json`, bucket `videos`) que ya tenía los US$0.1681 del Visual Test V2 — nunca arranca desde `$0`, nunca usa el ledger local en disco para este video+modo.
- **Reuso de las 3 imágenes ya aprobadas:** módulo nuevo `src/lib/video/long-form/production-ai-recreation.ts` (`resolveProductionAiRecreationImages`) — identifica los 11 shots AI_RECREATION vía `shot.source === "generated"`, lee los 3 ya aprobados (`b1-s4`/`b4-s2`/`b8-s5`) SOLO por lectura (`isApprovedVisualTestV2Shot` + el mecanismo ya existente de `visual-test-v2.ts`/`visual-test-v2-storage.ts`, nunca recalcula su identidad), y resuelve los 8 restantes con una identidad nueva construida directamente del `queryOrPrompt` del storyboard real, reutilizando el mismo `composePrompt()`/estilo/negativo compartido del Visual Test V2. Bypassea por completo `asset-resolver.ts` para estos shots (ese módulo enrutaba TODO `ken_burns_image` a Pexels, nunca a OpenAI — bug estructural preexistente, documentado y evitado con una resolución previa en el orquestador, nunca modificando `asset-resolver.ts` compartido).
- **Caché TTS durable entre ejecuciones separadas de GitHub Actions:** `src/lib/video/long-form/production-tts-cache.ts` — mismo diseño de `tts-cache.ts` (STARTED/COMPLETED/checksum) pero con Supabase Storage como backend, porque un runner de GitHub Actions es efímero (workspace nuevo en cada ejecución) y el caché en disco local solo protegía dentro de una misma ejecución.

**Workflow dedicado:** `.github/workflows/produce-video-001.yml` — EXCLUSIVO para VIDEO #001 (rutas de guion/storyboard/manifest fijas, nunca genera otro video). `workflow_dispatch` únicamente, con un input `confirm` que debe ser exactamente `PRODUCE_VIDEO_001` (cualquier otro valor nunca gasta dinero real). Un solo grupo de concurrencia (`atomivid-video-001-produccion-real`, `cancel-in-progress: false`) — imposible correr dos producciones reales a la vez. Dos jobs: `preflight` (gratuito, FAIL CLOSED) → `produce` (solo si `preflight` pasó Y `confirm` coincide exactamente). Sube 4 artifacts: MP4 final, `report.json`, `qc-report.json`, contact sheet PNG (`if: always()`, para poder diagnosticar incluso un run fallido).

**Preflight gratuito FAIL CLOSED:** `scripts/preflight-video-001.ts` — presencia (nunca valores) de `OPENAI_API_KEY`/`ELEVENLABS_API_KEY`/`PEXELS_API_KEY`/`SUPABASE_SERVICE_ROLE_KEY`/`NEXT_PUBLIC_SUPABASE_URL` (con `ELEVENLABS_VOICE_ID`/`ELEVENLABS_MODEL_ID` degradados a INFO, no bloqueantes — tienen default seguro en `src/lib/ai/voice.ts`, ya usado por el resto de la app); que los proveedores reales resuelvan sin caer a fixture (mismo código que usará la producción, cero llamadas pagas); accesibilidad de Supabase Storage/bucket/ledger; que el gasto acumulado del ledger sea **un piso** sobre el ya confirmado (nunca un valor exacto fijo — un exact-match se rompía en cuanto el ledger crecía con gasto real legítimo) y se mantenga dentro del hard stop; validación de los 3 shots aprobados como REUSE; conteos exactos contra la base canónica (45 shots, 11 AI_RECREATION totales/8 restantes, 4 STOCK_REAL, 13 beats TTS, 30 shots determinísticos con 0 placeholders posibles); proyección de costo máximo (peor caso, sin reuso) contra el hard stop de US$3.00.

**Herramienta de limpieza deliberada (uso único, con salvaguardas):** `scripts/clear-uncertain-shot-record.ts` + input opcional `clear_uncertain_shot_id` en el workflow — borra el registro `STARTED` huérfano de UN shot concreto SOLO cuando ya se confirmó por fuera (leyendo el log real) que el costo fue US$0.00. Nunca opera sobre los 3 shots aprobados, nunca borra un registro `COMPLETED`, recalcula el `idempotencyKey` exactamente como la producción real (nunca acepta uno a mano). Se usó UNA VEZ, para `b2-s3` (ver más abajo).

**Cronología real de ejecución (6 runs, gasto total incremental = 0, ningún run costó más de lo esperado):**
1. `35994929990` — preflight FALLA (falso positivo propio: `ELEVENLABS_VOICE_ID`/`MODEL_ID` tratados como obligatorios cuando tienen default seguro). **US$0 gastados.**
2. `35995538823` — preflight PASA. Sintetiza los 13 beats TTS reales exitosamente (ahora cacheados de forma durable). Falla reconstruyendo el shot timeline de `beat-7`: con la duración narrada REAL, `b7-s2` escaló a 22.608s, sobre el límite de 22s de `storyboard-shots.ts`. **Gasto real: ~US$0.4616 de TTS** (legítimo, cacheado, reutilizable).
3. `35996288699` — preflight FALLA (falso positivo propio: comparación EXACTA del gasto acumulado contra US$0.1681, que ya no aplicaba tras el gasto real de TTS del run 2). **US$0 gastados.** → corregido a un piso + hard-stop.
4. `35996818070` — preflight PASA (TTS reutilizado gratis). Alcanza generación REAL de imágenes OpenAI. Un prompt (`b2-s3`) es rechazado por moderación de OpenAI — **costo US$0 confirmado** (OpenAI nunca cobra un rechazo de moderación previo a generar), pero el registro `STARTED` quedaba huérfano sin limpiar, bloqueando cualquier reintento futuro de ese shot con un falso "consumo incierto".
5. `35997652508` — confirma (rápido, gratis) que el registro huérfano de `b2-s3` seguía bloqueando. Usado para diagnosticar el `shotId` exacto antes de decidir cómo proceder — el usuario eligió explícitamente "reintentar tal cual" antes de reescribir ningún prompt.
6. `35998403448` — **ÉXITO COMPLETO.** Limpieza de `b2-s3` (`clear_uncertain_shot_id=b2-s3`) → preflight PASA → TTS reutilizado (13/13, gratis) → 8 imágenes AI_RECREATION generadas (incluyendo `b2-s3`, que esta vez SÍ pasó moderación — el rechazo anterior fue una falsa alarma transitoria, no un problema real de contenido) → 4 shots STOCK_REAL resueltos vía Pexels → 30 shots determinísticos generados → render Remotion (1920x1080, ~24 min) → masterización de loudness → QC objetivo PASA → contact sheet generado → 4 artifacts subidos.

**Único cambio de storyboard de este checkpoint (error técnico claro, NO editorial):** `b7-s1`/`b7-s2` (beat-7) rebalanceados de 16/17 a 16.5/16.5 en `durationApprox` — mismo total, mismo orden, mismo `assetType`/`visualIntent`/`hybridClassification`/contenido, solo la proporción de pacing entre el diagrama y la tarjeta de texto, para que ambos queden bajo el límite de 22s con la duración narrada REAL (que resultó ~33% más larga que la planeada). Ningún otro shot, guion, tesis o storyboard fue tocado.

**Resultado final (MP4):**
- Ruta: `content/long-form/gobekli-tepe-001/output/gobekli-tepe-video-001-final.mp4` (en el workspace efímero del runner — el artifact descargable de GitHub Actions es la copia durable, retenida 90 días).
- Duración real: 756.448s (~12min 36s) — `targetDurationSeconds` del guion era solo un valor de fixture (180s) sin relación con la duración real narrada.
- 1920×1080, 30fps, con pista de video y audio — confirmado por ffprobe.
- 45/45 shots, `mode: "real"`, `isFixtureContent: false`, `paidApisCalled: true`.
- Loudness masterizado: -22.87 LUFS → -16.07 LUFS (objetivo -16), true peak -5.92 → -0.97 dBTP (objetivo -1.5).
- QC objetivo (`qc-video-001.ts`): `passed: true`, `hardFailures: []`, `silences: []` (cero silencios anómalos ≥4s detectados).
- Artifact de GitHub Actions: `gobekli-tepe-video-001` (run `35998403448`, ID `10808975834`), 87,501,666 bytes totales (MP4 + 3 reportes), retenido hasta 2026-12-23. URL: `https://github.com/hanzmo16-png/Atomivid/actions/runs/35998403448/artifacts/10808975834`.

**Desglose de costo real de VIDEO #001 (acumulado, ledger único de Supabase):**
- Visual Test V2 (3 imágenes ya aprobadas, checkpoint anterior): US$0.1681.
- TTS ElevenLabs (13/13 beats reales): ≈US$0.4616.
- Imágenes OpenAI nuevas (8/8 AI_RECREATION restantes, gpt-image-2, 1536x1024, medium): US$0.445685 exactos (`imageCostSpentUsd` del report.json).
- **Total acumulado: ≈US$1.0754** — muy por debajo del hard stop de US$3.00 (margen restante ≈US$1.9246).
- Pexels (4 shots STOCK_REAL): US$0 (siempre gratuito).
- 30 shots determinísticos (texto/diagrama/mapa): US$0 (sin llamada externa).

**NO se publicó a YouTube. NO se hizo ningún deploy adicional. El siguiente checkpoint es REVISIÓN HUMANA DEL MP4** (descargar el artifact y revisar consistencia visual de las 11 imágenes AI_RECREATION, anacronismos, manos/caras, legibilidad de diagramas/mapas, transiciones, antes de cualquier decisión de publicación).

**Validación de este checkpoint (todo el código, antes de cada push):** `npm run typecheck` limpio, `npm run lint` limpio, `npm run test:unit` 769/769 (incluye 2 tests nuevos: manejo de rechazo de moderación en `production-ai-recreation.test.ts`), `npm run build` exitoso (mismo único warning preexistente). `git diff --stat` confirma cero cambios en `remotion/VerticalReel.tsx`, `src/lib/video/generate-video.ts`, `src/app/api/generate/**` en cada commit — el pipeline 9:16 queda intacto.
